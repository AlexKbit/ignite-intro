...
public class ClusterExecuteService implements Service {
    ...
    private void execute(Job job) {
        cacheStore.startProgress(job.getTaskId());
        try {
            log.debug(String.format("Start execution job[%s] for task[%s]", job.getJobId(), job.getTaskId()));
            ignite.compute().callAsync(job).listen(t -> {
                log.debug(String.format("End execution job[%s] for task[%s]", job.getJobId(), job.getTaskId()));
                cacheStore.saveSuccess(job.getTaskId(), t.get());
            });
        } catch (Exception e) {
